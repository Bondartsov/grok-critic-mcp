...
| Публичный контракт: сигнатуры API, схемы, форматы файлов/сообщений, хранение | `critic_review`       |
| Нетривиальная логика: границы/индексы, парсинг, регэкспы, состояния, условия | `critic_review`       |
...
**Ревью не нужно (даже при большом диффе):** типовой код по готовому шаблону репо, конфиги, зависимости, документация, переименования, миграции/boilerplate, одноразовый экспериментальный код.

**Серая зона** (обычная последовательная логика без факторов риска, ~30–100 строк) — по умолчанию не звать: каждое скучное ревью обесценивает канал. Спорная глубина — это выбор `agent_count` (4 быстро / 16 глубоко), а не «звать/не звать». Полный чеклист — в [skill/SKILL.md](skill/SKILL.md).
...
В `AGENTS.md` (на уровне проекта) — обязательные точки вызова: после планирования (`architecture_review`), при неудачном баг-фиксе (`critic_review`), при риск-факторах изменения (`critic_review` / `security_audit`).
