...
| `grok-critic health [--ping] [--json]`                                                         | Быстрый статус конфига/store без сети; `--ping` — реальный запрос баланса и счётчиков                                                              |
...
При ориентировочных ценах Polza.AI ($2.6 / 1M input, $6.6 / 1M output) — **замеренные значения** (15.09.2026, cached 49–71%):

| Тип вызова                       |    Input |  Output | ≈ Стоимость |
| -------------------------------- | -------: | ------: | ----------: |
| Быстрое ревью (4 агента)         |  ~15 000 |  ~5 000 |      ~$0.08 |
| Полное ревью (16 агентов)        | ~270 000 | ~60 000 |      ~$1.10 |
| Followup по review_id (4 агента) |   ~7 000 |  ~1 300 |      ~$0.03 |
| Followup с полным текстом        |  ~25 000 | ~10 000 |      ~$0.13 |

> Input у 16 агентов много больше размера промпта: провайдер считает токены внутренней multi-agent оркестрации. Глубокое ревью — заметная трата; для рутины используйте `--agents 4`.
