...
- **Удалены** переменные `POLZA_PRICE_INPUT_PER_1M`, `POLZA_PRICE_OUTPUT_PER_1M`, `POLZA_DAILY_BUDGET_USD`. Если они остались в `.env`/окружении, запуск **не ломается** (`extra="ignore"`) — в лог пишется один DEPRECATED-warning с именами ключей (без значений); их стоит удалить.
