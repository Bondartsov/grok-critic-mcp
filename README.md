...
**Store диалогов на диске** (`db/reviews.json`, override — `POLZA_STORE_PATH`): MCP-сервер и CLI видят один store, поэтому `review_id` **переживает рестарты сервера** и работает из терминала; TTL записи — 24 ч с последнего обращения. Followup по `review_id` из CLI берёт ~7k input-токенов против ~25k при передаче полного текста.
