...
**Если MCP недоступен:** гейт ревью не отменяется. ZCode — субагент `reviewer` (read-only перепроверка альтернативным взглядом); в других клиентах — второй проход самопроверки по чеклисту (баги/edge-cases, SOLID, безопасность, недостающие тесты) с фиксацией в отчёте.
