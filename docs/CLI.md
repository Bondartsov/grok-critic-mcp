...
- Удалены переменные `POLZA_PRICE_INPUT_PER_1M`, `POLZA_PRICE_OUTPUT_PER_1M`, `POLZA_DAILY_BUDGET_USD`. Если они остались в `.env`/окружении — запуск не ломается, в лог пишется один DEPRECATED-warning с именами ключей (без значений); их стоит удалить: `python scripts/migrate_env_1_12.py .env .env.example` (dry-run) → `--apply`. Скрипт не выводит содержимое `.env`, только имена устаревших ключей и номера строк. Комментарии в реальном `.env` он не удаляет, а показывает их номера. Перед записью сверяет значения остальных ключей, при расхождении exit 2 и файл не тронут. Подробности — README, раздел «Миграция с 1.11.x на 1.12.0».
