...
> Версия документа: 1.11.1 (15.09.2026). CLI появился в v1.11.0, per-file store — в v1.11.1.
...
- Sandbox MCP (`POLZA_ALLOWED_READ_DIRS`) в CLI **не применяется**: файл читаете вы сами — это осознанное действие владельца. Если имя файла похоже на секрет (`.env*`, `*.key`, …), CLI предупредит в stderr, но не заблокирует.
...
| Sandbox `file_path`              | Да (allowed dirs + denylist)  | Нет — вы читаете файл сами (только warning на секреты)                             |
...
| `review_id не найден`                        | TTL 24ч истёк, store очищен или сервер старый (< v1.11.1) — используйте `--from <файл>`                                       |
