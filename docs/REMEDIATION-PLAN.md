...
> **Целевой релиз:** v1.9.0 (hardening) — все P0/P1; v1.9.1 — P2/P3. **v1.10.0** — вторая волна (SEC-02/03, REL-06, FEAT-*) по итогам ревью v1.9.0 (2026-08-21). **v1.11.0–v1.11.1** — CLI-фолбек, per-file ReviewStore (находка ревью `rev_4e2fb8bca326`). **v1.11.2** — третья волна (сверка 2026-09-16).

---

## v1.11.2 — третья волна (сверка 2026-09-16) — ✅ выполнено
...
- [x] `check_health` вручную против реального Polza.AI (smoke-test) — выполнено 2026-09-16 через MCP: Status ok, баланс получен
